...
var SuperRepo = function () {
    ...
    _createClass(SuperRepo, [{
        ...
        key: '_isDataUpToDate',
        value: function _isDataUpToDate(_localStore) {
            ...
            var isLimitExceeded = new Date().valueOf() - lastFetched > this.config.outOfDateAfter;
        ...
        value: function invalidateData() {
            ...
            return new Promise(function (_resolve) {
                ...
                _this6.storage.get(_this6.config.name).then(function (_prevData) {
                    var nextData = Object.assign({}, _prevData, {
                        isInvalid: true
                    });

                    _this6.storage.set(_this6.config.name, nextData).then(function () {
                        return _resolve({
                            prevData: _prevData,
                            nextData: nextData
                        });
        ...
        value: function clearData() {
            ...
            return new Promise(function (_resolve) {
                ...
                _this7.storage.get(_this7.config.name).then(function (_prevData) {
                    _this7.storage.set(_this7.config.name, null).then(function () {
        ...
        value: function getData() {
            ...
            return this.promise = new Promise(function (_resolve) {
                ...
                _this8.storage.get(config.name).then(function (_localData) {
                    if (_this8._isDataUpToDate(_localData)) {
                        _this8.promise = null;
                        _this8.isPromisePending = false;

                        _resolve(_localData.data);
                    } else {
                        config.request().then(_this8._normalizeData.bind(_this8)).then(_this8._mapData.bind(_this8)).then(function (_response) {
                            _this8._storeData(_response);
        ...
        value: function _initSyncInterval(_interval) {
            ...
            var interval = _interval < 1000 ? 1000 : _interval;
        ...
        value: function initSyncer() {
            ...
            this.storage.get(this.config.name).then(function (_localData) {
                if (_this10._isDataUpToDate(_localData)) {
                    var lastFetched = _localData.lastFetched;

                    var diff = new Date().valueOf() - lastFetched;

                    _this10.syncInterval = _this10._initSyncInterval(diff);

                    setTimeout(function () {
                        _this10.syncInterval = _this10._initSyncInterval(_this10.config.outOfDateAfter);
                    }, diff);
                } else {
                    _this10.getData().then(function (r) {
                        _this10.syncInterval = _this10._initSyncInterval(_this10.config.outOfDateAfter);
